...
- [x] 1.1 Build the `OpenTOPAS/OpenTOPAS-MeshGeom` extension against OpenTOPAS 4.2.p3. Build script at `scripts/build_meshgeom.sh` clones the repo, rebuilds TOPAS with both nBio + MeshGeom extensions, and installs to `~/topas_meshgeom_install`. Run `./scripts/build_meshgeom.sh` to execute.
- [x] 1.2 Resolve the `TsTetGeom` parameter contract from official example files. Verified: `Type = "TsTetGeom"`, supports both `Age`/`Sex` shortcut and explicit `NodeFile`/`EleFile`/`MaterialFile` paths; scorer `Quantity = "TsTetGeomScorer"` with `sv:Sc/.../ICRPMaterials = N "Organ" ...` for organ specification; `Material = "G4_WATER"` on the phantom component. Template uses explicit file paths (both approaches confirmed working per README and examples). Added `Report = "sum"` and conditional `ICRPMaterials` support.
