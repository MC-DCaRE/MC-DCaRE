...
try:
    from src.guilayers import (
        general_layer, main_menu_information_layer, function_layer, settings_information_layout,
        Hidden_layer, History_layer, imaging_protocol_layer, imaging_scan_layer,
        dicom_information_layer, dicom_file_layer, dicom_patient_layer,
        dicom_planned_layer, dicom_graphics_layer, CTDI_information_layer,
        CTDI_layer, Couch_layer, CTDI_blade_layer, CTDI_run_layer
    )
...
class TestGuiLayers:
    ...
    @pytest.mark.skipif(not GUI_IMPORTS_AVAILABLE,
    ...
    def test_all_gui_layers_defined(self):
        ...
        expected_layers = [
            'general_layer', 'main_menu_information_layer', 'function_layer',
            'settings_information_layout', 'Hidden_layer', 'History_layer',
            'imaging_protocol_layer', 'imaging_scan_layer', 'dicom_information_layer',
            'dicom_file_layer', 'dicom_patient_layer', 'dicom_planned_layer',
            'dicom_graphics_layer', 'CTDI_information_layer', 'CTDI_layer',
            'Couch_layer', 'CTDI_blade_layer', 'CTDI_run_layer'
        ]
        ...
        for layer_name in expected_layers:
            layer = globals().get(layer_name)
            assert hasattr(layer, '__class__'), f"Layer {layer_name} should be an object"
            ...
            assert hasattr(layer, 'Title') or hasattr(layer, 'get') or 'Column' in str(type(layer)), \
    ...
    def test_dicom_layers_exist(self):
        ...
        dicom_layer_names = [
            'dicom_information_layer', 'dicom_file_layer', 'dicom_patient_layer',
            'dicom_planned_layer', 'dicom_graphics_layer'
        ]
    ...
    def test_ctdi_layers_exist(self):
        ...
        ctdi_layer_names = [
            'CTDI_information_layer', 'CTDI_layer', 'Couch_layer',
            'CTDI_blade_layer', 'CTDI_run_layer'
        ]
    ...
    def test_layer_titles_are_descriptive(self):
        ...
        layer_mappings = [
            ...
            ('dicom_information_layer', 'Instructions on the usage of the DICOM adjustments'),
            ...
            ('CTDI_information_layer', 'Instructions on the usage of the CTDI phantom parameters'),
