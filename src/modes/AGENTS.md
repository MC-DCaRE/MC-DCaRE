...
- `phantom_mode.py` -- ICRP 145 reference phantom dose. Builds contexts with phantom data path, sex selection (AM/AF), supine placement offsets, couch params, and organ-dose output filename. Single execution run with headsource + phantomICRP145 combined. Uses `SimulationRunner.run_phantom()`.
