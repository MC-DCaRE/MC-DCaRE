...
| `TOPAS_includeFiles/fullfan.txt` | Legacy full-fan bow-tie: aluminum CSG trapezoid wedges (used when `imaging.legacy_bowtie=True`, the default) |
| `TOPAS_includeFiles/halffan.txt` | Legacy half-fan bow-tie: offset aluminum CSG wedges |
| `TOPAS_includeFiles/bowtie_ff.txt` | TsCAD mesh full-fan bow-tie (measured Inbum STL): `Rotation` parent @ 18 cm SDD, `RotZ=-90`. Used when `imaging.legacy_bowtie=False` |
| `TOPAS_includeFiles/bowtie_hf.txt` | TsCAD mesh half-fan bow-tie (one-sided crop of the full-fan STL; approximation) |
| `TOPAS_includeFiles/fullfan.stl` / `halffan.stl` | Binary STLs consumed by `bowtie_ff.txt`/`bowtie_hf.txt` via `TsCAD`. Generated by `tools/process_bowtie_stl.py` |
