...
iso = [12; 5]; %Isocentre coordinate (x,y), relative to the geometrical centre of the phantom in the transverse slice (cm)
z = 30; %Sup-Inf offset (cm)(located z=30cm put sup edge of field under diapragm) 
...
finalAngle = 160; %Final gantry angle in degrees
...
subFieldCoords = [0, 0, 0, 0, 0, 0, 0 ; -13.25, -10, -5, 0, 5, 10, 13.25]; %Sub-field coordinates defined a gantry 0, eg. [0, 0, 0, ... ; y1, y2, y3, ...] (coordinates of centres of sub-fields)
%for the subfields, the blades go between X1=14cm and X2=14cm, this
%means that we are 14cm below the iso, and 14cm above iso. We have
%modelled the +13.25cm centre below iso (0.75cm either side of the centre so 
%down to +14cm in PCXMC) and anterior to iso to 14cm 
subFieldWidths = [1.5,5,5,5,5,5,1.5]; %Width of each sub-field at each sub-field coordinate (cm) (X-direction blades, <<<<CHECK THIS>>>>>)
width = 21.4; %Width of the field at isocentre in the sup-inf direction (cm) this is Y-blades on the source
%patient had 17cm reconstructed, 
...
%Patient geometry details
%87kg, ~165cm

pWidth = 19.0; %Dual variable: If headScan = false , this represents half the phantom's body width (cm). If headScan = true, anterior radius of phantom's head (cm)
pDepth = 11.0; %Phantom's body depth (cm) (half)
pWidthTemp = pWidth; % Value as above for destructive use.
pDepthTemp = pDepth; % Value as above for destructive use.
pHeadRadii2 = 4.75; %If headScan = true, posterior radius of phantoms head (cm).
height = 165; %Phantom's height (cm)
mass = 87; %Phantom's mass (kg)
