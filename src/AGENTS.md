...
**Bow-tie wedge varies in World Z.** With `RotZ=-90` (and `RotY=0`) the STL thin axis lies along the beam (Y), the wedge falls in World Z, and the extrusion is uniform in World X. Verified by a 2D X×Z isocenter kerma map + visual 3-view (`docs/bowtie_validation/plots/`). The `validate_bowtie` scorer in `ctdi_phsp_score.j2` (a `TrackLengthEstimator` on a thin air slab at isocenter) bins Z with a wide Z-field (`user_field_x` ≥ 32 cm) so the collimator does not cut the profile; `tools/convert_bowtie_profile.py` + `tools/validate_bowtie.py` do the comparison. Measured Head-FF CAX HVL = 7.37 mmAl (RaySafe Oct-2023 workbook `research/2023 - CBCT Dose PCXMC/New results Oct 2023 (Spotlight, Raysafe X2).xlsx`, literature anchor).

**CAX HVL validation (2026-08-18):** the `validate_bowtie` block also scores the post-bow-tie CAX energy spectrum (`CaxSlab` + `Sc/CaxSpectrum` Fluence scorer, 150×1 keV bins); `tools/compute_cax_hvl.py` folds it with NIST Al via `PhaseSpaceAnalyzer.compute_hvl_mm_al`. Head FF: TsCAD 7.80 vs 7.37 measured (+0.43, passes ~0.5 mmAl tol), legacy CSG 8.26 (fails). Pelvis HF: TsCAD 8.96 vs 8.06 (+0.90). No-bow-tie controls (7.96/8.90) prove the residual is base filtration (Ti BHF + inherent Al), not the bow-tie asset. The energy-resolved HVL(Z) map matches measurement to 0.3-0.5 mmAl off-axis (|Z| ≥ 7 cm); the short central plateau is the known under-thin STL CAX (see below). Full tables: `docs/bowtie_validation/README.md`.
