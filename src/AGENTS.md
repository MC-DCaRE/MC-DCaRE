...
- **DCF for DTM (body tissue)** cannot be derived from CTDI measurements in *air* chambers (Count_in_Bin=0). Two viable routes: (1) water-filled chamber plugs via `water_chamber_enabled` — the `_water_dtm` scorers register non-zero dose (see note above), enabling a CTDI-derived DTM DCF; (2) the reference effective dose, `DCF_dtm = E_reference / E_raw_MC`. With the unified normalization (`fix-normalization-unified-dcf`), Pelvis (125kV Half Fan) direct-beam: `DCF_dtm = 4.2 / 152.13 = 0.02761` (stored in `calibration.yaml` as an interim value).
