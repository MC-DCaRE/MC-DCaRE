...
class TestRunTopas:
    ...
    @patch("src.simulation_runner.subprocess.run")
    def test_prints_ran_after_execution(
...
class TestRunCtdi:
    ...
    def test_calls_run_topas_for_each_command(self, mock_pool_class: MagicMock) -> None:
        ...
        commands: List[Tuple[str, str]] = [
            (topas_path + " " + rundatadir + "/ChamberPlugCentre.txt", rundatadir),
            (topas_path + " " + rundatadir + "/ChamberPlugTop.txt", rundatadir),
            (topas_path + " " + rundatadir + "/ChamberPlugBottom.txt", rundatadir),
        ]
