...
%Thorax, 360 degrees, startsource 180 - stopsource 180 on the linac only, this
...
%For these sub-fields, as the PCXMC software is shit, we need to break down
%the sub-fields so that they can represent the profile across the blades.
%the y1, y2 etc are misleading here. they represent a starting point that
%is assuming the gantry is at PCXMC zero (Lt Lateral source) and the 
%y direction is ant/post which is across the bow-tie filter. 
%ant/post where -Y is Anterior, +Y is Posterior. +X is Lt lateral, -X is 
%Rt Lat. 
%starting at gantry 0 in PCXMC world in this case is with the source at 
%real linac gantry 90 degrees.
%for thorax, the panel starts at linac gantry 270 with the panel relocating 
%up towards the cieling or -Y direction. THis will go beyond the phanom
%boundaries but is needed to define the say AP/PA beams where the phantom
%is wider laterally than it is thick A/P
subFieldCoords = [0, 0, 0, 0, 0, 0, 0 ; -2.75, 0, -5, -10, -15, -20, -25]; %Sub-field coordinates defined a gantry 0, eg. [0, 0, 0, ... ; y1, y2, y3, ...] (coordinates of centres of sub-fields)
subFieldWidths = [0.5,5,5,5,5,5,5]; %Width of each sub-field at each sub-field coordinate (cm) (X-direction blades, <<<<CHECK THIS>>>>>)
width = 21.4; %Width of the field at isocentre in the sup-inf direction (cm) this is Y-blades on the source <<<<CHECK THIS>>>>>
kerma = (numAnglesTrue/numAnglesSimmed).*[0.0175,0.0170,0.0137,0.0062,0.0013,0.0013,0.0013]; %Air kerma per projection at each sub-field coordinate (mGy).
filtration = [8.004,8.083,8.581,9.911,8.8917,8.8917,8.8917]; %Total filtration at each sub-field coordinate (mm Al)

%Phantom geometry details
%average male
%body, 1.8m tall, 88kg, A/P dimensions of body 22cm, Lateral 35cm

pWidth = 17; %Dual variable: If headScan = false , this represents the phantom's body width (cm). If headScan = true, anterior radius of phantom's head (cm)
pDepth = 10.5; %Phantom's body depth (cm), need to make these both the same for head???? i think they do need to be the same for head.
...
if plotting==true
    ...
    %changed the plots so that the coordinate system of PCXMC shows up on
    %them, it's not immediately obvious
    xlabel('-X=RtLat,   X-Position (cm),   +X=LtLat')
    ylabel('-Y=Ant, Y-Position (cm), +Y=Post')
