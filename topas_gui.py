...
def reset_tmp():
    ...
    original_headsource_file_path = (
        path + "/src/boilerplates/headsourcecode_boilerplate.txt"
    )
    ...
    original_dicom_file_path = (
        path + "/src/boilerplates/TOPAS_includeFiles/patientDICOM.txt"
    )
    ...
    original_CTDI_16_file_path = (
        path + "/src/boilerplates/TOPAS_includeFiles/CTDIphantom_16.txt"
    )
    ...
    original_CTDI_32_file_path = (
        path + "/src/boilerplates/TOPAS_includeFiles/CTDIphantom_32.txt"
    )
...
initiate = True
reset_tmp()
window["-G4FOLDERNAME-"].bind(
    "<Return>", "_ENTER"
)  # for quick and dirty debuggin with G4 enter, remove for actual release
...
while True:
    ...
    if event == "-DICOMRP-":
        ...
        if dcmread(values["-DICOMRP-"]).PatientID == values["-PATID-"]:
            try:
                isocentre_coors = (
                    dcmread(values["-DICOMRP-"])
                    .BeamSequence[0]
                    .ControlPointSequence[0]
                    .IsocenterPosition
                )
                values["-DICOM_ISOX-"] = (
                    str(round(isocentre_coors[0], 5)) + " mm"
                )  # figure out how to get units form dicom
    ...
    if event == "-DICOM_RUN-":
        ...
        try:
            ...
            float_anode_voltage, unit_anode_voltage = quantity_unit_stripper(
                values["-IMAGEVOLTAGE-"]
            )
