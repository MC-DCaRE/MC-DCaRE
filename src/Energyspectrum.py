...
def generate_new_topas_beam_profile(
    anode_voltage: float, exposure: float, Histories: str, path
):
    s = sp.Spek(
        kvp=anode_voltage, th=14, mas=exposure, dk=0.2, z=0.1
    ...
    karr, spkarr = s.get_spectrum(
        edges=False, diff=False
    )  # returns an array of photon energies and its corresponding fluence
    ...
    convertedFile = (
        ...
        + str(energySpectrum.size)
        + "\n "
        + str(energySpectrum)[1:-1]
        + " keV \n"
        + "\n uv:So/beam/BeamEnergySpectrumWeights = "
        + str(weightedFluence.size)
        + "\n "
...
if __name__ == "__main__":
    ...
    generate_new_topas_beam_profile(
        anode_voltage, exposure, Histories, "/home/jkgan/MC-DCaRE"
    )
