...
def extract_dose_from_file(file_path: Path) -> Optional[float]:
    ...
    try:
        ...
        if dose_values:
            ...
            logger.info(
                f"Extracted {len(dose_values)} dose values from {file_path}, total: {total_dose:.6e}"
            )
            return total_dose
        else:
...
def calculate_ctdi_w(peripheral_doses: List[float], center_dose: float) -> float:
    ...
    logger.info(
        f"Peripheral average: {peripheral_avg:.6e}, Center dose: {center_dose:.6e}"
    )
...
def process_file_type(
    chamber_files: Dict[str, Path], file_type: str, calibration_factor: float = 1.0
) -> Optional[Dict]:
    """
    ...
    for position in PERIPHERAL_POSITIONS:
        if position in chamber_files:
            ...
            if dose is not None:
                ...
                logger.info(
                    f"{position} dose: {scaled_dose:.6e} Gy (scaled by factor {calibration_factor})"
                )
            else:
    ...
    if CENTER_POSITION in chamber_files:
        ...
        if dose is not None:
            ...
            logger.info(
                f"Center dose: {center_dose:.6e} Gy (scaled by factor {calibration_factor})"
            )
        else:
...
def main(
    ...
    output_file: Optional[str] = typer.Option(
        None, "--output", "-o", help="Output CSV filename (default: CTDIw_results.csv)"
    ),
) -> None:
    """
    Calculate CTDI_w values from ChamberPlug CSV files.
    ...
    for file_type in FILE_TYPES:
        if chamber_files[file_type]:
            result = process_file_type(
                chamber_files[file_type], file_type, calibration_factor
            )
            if result:
    ...
    if not results:
        console.print(
            "❌ No results generated. Check file availability and format.", style="red"
        )
        raise typer.Exit(1)
    ...
    for result in results:
        console.print(
            f"  {result['FileType']}: CTDI_w = {result['CTDI_w']:.6e} Gy", style="cyan"
        )
