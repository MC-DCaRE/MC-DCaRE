...
pWidth = 8.5; %Dual variable: If headScan = false , this represents the phantom's body width (cm). If headScan = true, anterior radius of phantom's head (cm)
pDepth = 8.5; %Phantom's body depth (cm), need to make these both the same for head???? i think they do need to be the same for head.
pWidthTemp = pWidth; % Value as above for destructive use.
pDepthTemp = pDepth; % Value as above for destructive use.
pHeadRadii2 = 4.5; %If headScan = true, posterior radius of phantoms head (cm).
height = 150; %Phantom's height (cm)
mass = 50; %Phantom's mass (kg)
