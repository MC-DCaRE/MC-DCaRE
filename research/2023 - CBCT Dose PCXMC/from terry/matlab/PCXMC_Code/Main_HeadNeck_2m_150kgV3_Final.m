...
%head, 360 degrees, startsource 180 - stopsource 180 on the linac only, this
%translates into startangle 0 - stopangle 360 on the PCXMC coordinate
%system. total of 900 projections
...
%180 is RtLateral
%270 is AP
startingAngle = 0; %Starting gantry angle in degrees
finalAngle = 360; %Final gantry angle in degrees
numAnglesSimmed = 20; %Number of projections/angles to be used in the simulation
numAnglesTrue = 900; %Number of projections/angles in the actual scan
oblique = 0; %Used by PCXMC to simulate oblique angles.
subFieldCoords = [0, 0, 0, 0, 0, 0, 0 ; 3.25, 0, -5, -10, -15, -20, -25]; %Sub-field coordinates defined a gantry 0, eg. [0, 0, 0, ... ; y1, y2, y3, ...] (coordinates of centres of sub-fields)
%for the subfields, the blades go between X1=24.7cm and X2=3.3cm, this
%means that we are 3.3cm below the iso, and 24.7cm above iso. We have
%modelled the 3.25cm centre below iso (+0.75cm either side of the centre so 
%down to +4cm in PCXMC) and anterior to iso to 25cm (+2.5cm) 
subFieldWidths = [1.5,5,5,5,5,5,5]; %Width of each sub-field at each sub-field coordinate (cm) (X-direction blades, <<<<CHECK THIS>>>>>)
%Note for the line above, when starting at PCXMC angle zero (LtLateral), 
%the panel moves towards the patients anterior (assuming supine) which in 
%PCXMC is in the -Y direction. this means when looking at the kV source if
%located supine LfLateral, the thin portion of the half-fan is facing
%downwards with the thicker portion of the half-fan at the top.
width = 21.4; %Width of the field at isocentre in the sup-inf direction (cm) this is Y-blades on the source <<<<CHECK THIS>>>>>
%the head and neck anatomy based on the patient in PCXMC requires a field 
%of 28cm Y-direction and a cenrte of z=83cm. 
%HOWEVER, speaking to the RT's, H&N isn't done with an extended
%field and the Y-blades are set to 21.4cm. Hence will choose fields that
%expose from ~eyes down (no lens in PCXMC) and include the bottom of the
%thyroid.
kerma = (numAnglesTrue/numAnglesSimmed).*[0.0058,0.0057,0.0046,0.0021,0.0004,0.0004,0.0004]; %Air kerma per projection at each sub-field coordinate (mGy).
filtration = [8.00,8.08,8.58,9.91,8.89,8.89,8.89]; %Total filtration at each sub-field coordinate (mm Al)

%Phantom geometry details
%average male
%for head, 2m tall, 150kg, anterior radius of head 13.5cm, posterior radius
%13.5cm. however the lateral profile of the larger radius of the head does
%reduce in size meaning choosing 7.5cm will put reference points out of the
%phantom.

pWidth = 13.3; %Dual variable: If headScan = false , this represents the phantom's body width (cm). If headScan = true, anterior radius of phantom's head (cm)
pDepth = 13.3; %Phantom's body depth (cm), need to make these both the same for head???? i think they do need to be the same for head.
pWidthTemp = pWidth; % Value as above for destructive use.
pDepthTemp = pDepth; % Value as above for destructive use.
pHeadRadii2 = 7; %If headScan = true, posterior radius of phantoms head (cm).
height = 200; %Phantom's height (cm)
mass = 150; %Phantom's mass (kg)
...
if mod(finalAngle-startingAngle,360) == 0 %If the rotation is a closed loop the final angle should not equal initial angle.
    ...
    if gantry_direction == 0
        if finalAngle>startingAngle 
            ...
            dA = ((finalAngle+360)-startingAngle)/(numAnglesSimmed-1); %Gantry angle interval
            angles = startingAngle:dA:(finalAngle+360); %Gantry angle array
        end %if    
    else
        if finalAngle>startingAngle
            finalAngle = finalAngle-360;
            dA = (startingAngle-finalAngle)/(numAnglesSimmed-1);
        else %if
            %finalAngle = finalAngle-360;
            dA = (startingAngle-finalAngle)/(numAnglesSimmed-1);        
        end %if
...
% now uses matlab function, not Aarons given WrapTo360 function
...
PCXMCInput(:,3) = round(anglesFinal(:,1),3); %Projection (AP,PA,LATL,LATR or num.angle)
%Round here just limits the angles to three decimal places as the PCXMC
%code uses the angle for naming of files and they end up really long. I'm
%ignoring that this may not be a num.angle
...
PCXMCInput(:,1) = (1:size(PCXMCInput,1))'; %Hospital, should be number 1-size of array otherise an error gets thrown in PCXMC
...
%the other hot tip is to put something in the second column, this is called
%'Examination', this is just for tidying up the billions of files you get
%in the folder structure in PCXMC
