    ...
    with open(mat_path) as f:
        for line in f:
            ...
            if not line:
                continue
            if line.startswith("C "):
                # Material header: "C <organ_name> <density> g/cm3".
                flush()
                parts = line.split()
...
def validate_material_coverage(
    voxel_grid: np.ndarray, materials: Dict[int, dict]
) -> None:
    """Pre-flight: every grid mat_id must resolve to an emittable material.

    Raises ``ValueError`` listing any mat_id that is missing from ``materials``
    or whose elements are all unmapped in ``_Z_TO_NAME`` (which would leave a
    VoxelMaterials entry with no TOPAS material definition -> TOPAS crash).
    mat_id 0 (air / unfilled) is exempt.
    """
    grid_ids = set(int(m) for m in np.unique(voxel_grid) if int(m) != 0)
    missing = sorted(i for i in grid_ids if i not in materials)
    no_elements = sorted(
        i for i in grid_ids if i in materials and not materials[i]["elements"]
    )
    unknown_z: Dict[int, list] = {}
    for i in grid_ids:
        if i in materials and materials[i]["elements"]:
            bad = [z for z, _ in materials[i]["elements"] if z not in _Z_TO_NAME]
            if bad and not any(z in _Z_TO_NAME for z, _ in materials[i]["elements"]):
                unknown_z[i] = sorted(set(bad))
    if missing or no_elements or unknown_z:
        msgs = []
        if missing:
            msgs.append("mat_id(s) absent from .material file: %s" % missing)
        if no_elements:
            msgs.append("mat_id(s) with no elements parsed: %s" % no_elements)
        if unknown_z:
            msgs.append(
                "mat_id(s) with only unmapped Z (not in _Z_TO_NAME): %s" % unknown_z
            )
        raise ValueError(
            "Material coverage check failed; TOPAS would crash on these IDs:\n  "
            + "\n  ".join(msgs)
        )
...
def main():
    ...
    # Pre-flight: fail loudly (with details) instead of crashing in TOPAS on an
    # unmapped material/Z once the VoxelMaterials vector references it.
    validate_material_coverage(voxel_grid, materials_full)
