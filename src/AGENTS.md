...
- **TLE is the only scorer that works in LayeredMassGeometry (LMG) parallel worlds.** DTM and DTW give zero because collision-based scorers require physics interactions inside the parallel world volume, but particles track in the mass geometry (PMMA) — the scorer's sensitive detector is never triggered. The G4ParallelWorldProcess with `SetLayeredMaterialFlag` replaces the material for physics but not for scoring volume assignment.
