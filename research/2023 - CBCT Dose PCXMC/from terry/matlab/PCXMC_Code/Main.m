...
numAnglesSimmed = 360; %Number of projections/angles to be used in the simulation
numAnglesTrue = 360; %Number of projections/angles in the actual scan
oblique = 0; %Used by PCXMC to simulate oblique angles.
subFieldCoords = [0, 0, 0, 0,0, 0, 0 ; -15 -10, -5, 0, 5, 10, 15 ]; %Sub-field coordinates defined a gantry 0, eg. [0, 0, 0, ... ; y1, y2, y3, ...]
...
pHeadRadii2 = 13; %If headScan = true, posterior radius of phantoms head (cm).
...
PCXMCInput(:,3) = anglesFinal(:,1); 
PCXMCInput(:,4) = oblique; 
PCXMCInput(:,5) = UID; 
PCXMCInput(:,6) = height; 
PCXMCInput(:,7) = mass; 
PCXMCInput(:,8) = age; 
PCXMCInput(:,9) = kV; 
PCXMCInput(:,10) = FiltFinal(:,1); 
PCXMCInput(:,11) = 0; 
PCXMCInput(:,12) = FRD; 
PCXMCInput(:,13) = sFWFinal(:,1); 
PCXMCInput(:,14) = width;
PCXMCInput(:,15) = XCoordsFinal(:,1); 
PCXMCInput(:,16) = YCoordsFinal(:,1); 
PCXMCInput(:,17) = z; 
PCXMCInput(:,18) = arms; 
PCXMCInput(:,20) = KermaFinal(:,1);
PCXMCInput(:,1) = 0;
PCXMCInput(:,2) = 0;
