...
class TestImagingModesLookup:
    ...
    def test_selection_keys_structure(self):
        ...
        expected_keys = [
            'Rotation Rate', 'kVp', 'exposure', 'Fan', 'timeend',
            'FIELD_X1', 'FIELD_X2', 'FIELD_Y1', 'FIELD_Y2',
            'BLADE_X1', 'BLADE_X2', 'BLADE_Y1', 'BLADE_Y2'
        ]
        
        assert len(selection) == len(expected_keys), f"Expected {len(expected_keys)} keys, got {len(selection)}"
    ...
    def test_rotation_rate_values(self):
        ...
        positive_rates = [r for r in rotation_rates if 'deg/s' in r and not r.startswith('-')]
        negative_rates = [r for r in rotation_rates if 'deg/s' in r and r.startswith('-')]
    ...
    def test_exposure_values(self):
        ...
        expected_exposures = ['100 mAs', '150 mAs', '210 mAs', '270 mAs', '750 mAs', '1688 mAs']
    ...
    def test_time_end_values(self):
        ...
        for expected_time in expected_times:
            assert expected_time in time_end_values, f"Missing time end: {expected_time}"
    ...
    def test_field_values(self):
        ...
        expected_fields = ['14 cm', '3.3 cm', '13.2 cm', '24.7 cm', '501 s']  # Include time value
        for expected_field in expected_fields:
            assert expected_field in field_values, f"Missing field size: {expected_field}"
    ...
    def test_blade_values(self):
        ...
        for blade in blade_values:
            ...
            assert isinstance(float(numeric_part), float), f"Invalid blade value: {blade}"
    ...
    def test_mode_naming_consistency(self):
        ...
        for mode_name in imaging_modes_lookup.keys():
            ...
            assert len(parts) >= 2, f"Mode name should have at least 2 parts: {mode_name}"
            ...
            if mode_name.startswith('CBCT'):
                ...
                assert mode_name.startswith('CBCT '), f"CBCT mode should start with 'CBCT ': {mode_name}"
                
                # Extract direction from part after "CBCT "
                cbct_part = mode_name[5:]  # Remove "CBCT "
                cbct_parts = cbct_part.split('_')
                assert len(cbct_parts) >= 2, f"CBCT mode should have direction and body part: {mode_name}"
                
                # Direction should be Clockwise or Anticlockwise
                valid_directions = ['Clockwise', 'Anticlockwise']
                assert cbct_parts[0] in valid_directions, f"Invalid CBCT direction: {cbct_parts[0]}"
                
                # Body part should be valid
                valid_body_parts = ['Image Gently', 'Head', 'Short Thorax', 'Spotlight', 'Thorax', 'Pelvis', 'Pelvis Large']
                body_part = '_'.join(cbct_parts[1:])  # Join remaining parts for body part with size
                assert body_part in valid_body_parts, f"Invalid CBCT body part: {body_part}"
                
            elif mode_name.startswith('kV-kV'):
                # kV-kV modes have format: "kV-kV_BodyPart" or "kV-kV_BodyPart_Size"
                assert parts[0] == 'kV-kV', f"kV-kV mode should start with 'kV-kV': {mode_name}"
                
                # Body part should be valid
                valid_body_parts = ['Image Gently', 'Head', 'Short Thorax', 'Spotlight', 'Thorax', 'Pelvis', 'Pelvis Large']
                body_part = '_'.join(parts[1:])  # Join remaining parts for body part with size
                assert body_part in valid_body_parts, f"Invalid kV-kV body part: {body_part}"
    ...
    def test_lookup_table_completeness(self):
        ...
        non_selection_modes = [k for k in imaging_modes_lookup.keys() if k != 'selection']
