...
finalAngle = 20; %Final gantry angle in degrees
numAnglesSimmed = 2; %Number of projections/angles to be used in the simulation
numAnglesTrue = 900; %Number of projections/angles in the actual scan
oblique = 0; %Used by PCXMC to simulate oblique angles.
subFieldCoords = [0, 0, 0; 0, 5, 20]; %Sub-field coordinates defined a gantry 0, eg. [0, 0, 0, ... ; y1, y2, y3, ...] (coordinates of centres of sub-fields)
subFieldWidths = [2.5,2.5,2.5]; %Width of each sub-field at each sub-field coordinate (cm) (X-direction blades, <<<<CHECK THIS>>>>>)
width = 21.4; %Width of the field at isocentre in the sup-inf direction (cm) this is Y-blades on the source <<<<CHECK THIS>>>>>
kerma = (numAnglesTrue/numAnglesSimmed).*[0.0175, 0.0175, 0.0175]; %Air kerma per projection at each sub-field coordinate (mGy).
filtration = [8.004, 8.004, 8.004]; %Total filtration at each sub-field coordinate (mm Al)

%Phantom geometry details
%average male
%body, 1.8m tall, 88kg, A/P dimensions of body 22cm, Lateral 35cm

pWidth = 21; %Dual variable: If headScan = false , this represents the phantom's body width (cm). If headScan = true, anterior radius of phantom's head (cm)
