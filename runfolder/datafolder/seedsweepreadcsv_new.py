...
# print(os.getcwd())
sys.path.insert(0, '../..')
from generate_allproc import boundaries_name_list, boundaries_list
databs = pd.DataFrame()
...
for names in protocolnames:
	for component_name, values in zip(boundaries_name_list*len([*product(*(my_arange(*b) for b in boundaries_list))]),product(*(my_arange(*b) for b in boundaries_list))):
		...
		if names == "Top":
			tablename = centrecsvname.replace('Tphd_','')
		elif names == "Centre":
			tablename = centrecsvname.replace('Cntrhd_','')
		elif names == "Bottom":
			tablename = centrecsvname.replace('Bttmhd_','')
		elif names == "Left":
			tablename = centrecsvname.replace('Lfthd_','')
		elif names == "Right":
			tablename = centrecsvname.replace('Rghthd_','')
		
		#if want other quantity change to dtm or dtw in the string below
		filename = centrecsvname+"ChamberPlug_tle.csv"
		print(filename)    
		if os.path.isfile(filename):
			# print("isfile")
			df = pd.read_csv(filename,skiprows = 8,names = ['X', 'Y', 'Z', 'Dose (Gy)'])     
			# totaldose = df['Dose (Gy)'].sum()
			# df = pd.read_csv(centrecsvname+ ".csv",skiprows = 8, names = ['X', 'Y', 'Z', 'Dose (Gy)'])
			totaldose = df['Dose (Gy)'].sum()
			print(totaldose)
